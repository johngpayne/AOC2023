...
impl Map {
    ...
    fn longest_route(&self) -> usize {
                    ...
                    let node = [0, 1, 2, 3].map(|dir_index| {
                        if !self.can_move(pos, dir_index) {
                            ...
                            loop {
                                ...
                                len += 1;
                                if pos == self.end || pos == self.start || self.get(pos).count_ones() > 2 {
                                    ret = Some((pos, len));
                                    break;
                                }
                                if self.get(pos).count_ones() == 1 && !self.can_move(pos, current_index) {
                                    break;
                                }
                                current_index = (0..4)
                                    .find(|&test_index| {
        ...
        let pos_to_index = nodes.iter().enumerate().map(|(index, (pos, _))| {
            (*pos, index as u8)
        }).collect::<FxHashMap<_, _>>();
        
        // get [[maybe (index, len)>]]
        let index_nodes = nodes.values().map(|node_entries| {
            node_entries.map(|entry| entry.map(|(to_pos, len)| (pos_to_index.get(&to_pos).unwrap(), len)))
        }).collect_vec();
...
pub fn solve(input: &str) -> String {
    let grid_ch = input
        ...
        .collect_vec();
    format!("{}/{}", Map::new(&grid_ch, false).longest_route(), Map::new(&grid_ch, true).longest_route())
}
